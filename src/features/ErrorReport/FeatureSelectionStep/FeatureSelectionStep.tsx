...
const FeatureSelectionStep = ({ nextStep, setBody }: Props) => {
  ...
  const handleFetchData = async () => {
    setLoading(true)
    const {callbacks} = PRIVATE_ROUTES.find(c => c.name === page) as ErrorFeature
    if (withCodeInput) {
      const code = parser.parseSubjectCode(subjectCode) as number
      const populatedCallbacks = callbacks.map((c: (n: number) => Promise<string>) => () => c(code))
  ...
  return (
    <Container>
      <Text italic marginTop='8px' marginBottom='4px'>Onde o erro aconteceu?</Text>
      <StyledPicker
          selectedValue={page}
        ...
        {withCodeInput && (
          <>
            <Text  italic marginTop='8px' marginBottom='4px'>Insira o código da disciplina</Text>
            <TextInput onChangeText={setSubjectCode} placeholder="Ex.: IME-01388"></TextInput>
          </>
        )}
      <NextBtn size='small' fullWidth onPress={handleOnPress} loading={loading} disabled={!page}>Próximo</NextBtn>
    </Container>
  );
}
